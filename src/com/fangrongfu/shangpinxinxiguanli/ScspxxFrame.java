...
public class ScspxxFrame extends JFrame{
	...
	public ScspxxFrame() {
		...
		querenButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					Dao.scspxx(idStr,nameStr);//函数功能：增加商品信息
					JOptionPane.showMessageDialog(ScspxxFrame.this, "删除商品信息成功","提示", JOptionPane.INFORMATION_MESSAGE);
					Dao.scsprecord(LoginDialog.userid,idStr);//增加删除商品信息操作的历史记录，要有商品id
					//}
				}catch(Exception e1) {
					e1.printStackTrace();
				}
		...
		quxiaoButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				int result=JOptionPane.showConfirmDialog(ScspxxFrame.this, "是否确认退出？", "提示", JOptionPane.YES_NO_OPTION);
				if(result==JOptionPane.YES_OPTION) {
					ScspxxFrame.this.dispose();
				}
				else if(result==JOptionPane.NO_OPTION){
					ScspxxFrame.this.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
				}
			}
		...
		this.setTitle("删除商品信息界面");
		this.setIconImage(getToolkit().getImage(getClass().getResource("/resources/deletegoods.PNG")));
