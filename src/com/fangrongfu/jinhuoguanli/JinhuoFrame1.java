...
public class JinhuoFrame1 extends JFrame{

	JLabel numberLabel;//“进货数量”标签

	JTextField numberField;//“进货数量”文本框
	...
	public JinhuoFrame1(String id,String num,JTable table,String Str,Vector tableValues) {

		numberLabel=new JLabel("进货数量");//“进货数量”标签
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					if(isNumeric(tianjiaNum)==true) {
						...
						tableValues.addElement(Dao.scchdxx(id, tianjiaNum));//将缺货的商品信息添加到缺货单

						table.getModel().setValueAt(newNum, table.getSelectedRow(), 8);//修改表格
						table.updateUI();//更新表格
						JOptionPane.showMessageDialog(JinhuoFrame1.this, "已成功添加该商品到进货信息核对表","提示", JOptionPane.INFORMATION_MESSAGE);
						setVisible(false);
					}else {
						JOptionPane.showMessageDialog(JinhuoFrame1.this,"输入的不是数字，请输入数字","错误",JOptionPane.ERROR_MESSAGE);
					}
