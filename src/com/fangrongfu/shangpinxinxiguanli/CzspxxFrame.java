...
public class CzspxxFrame extends JFrame{
	...
	public CzspxxFrame() {
		...
		querenButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					Dao.czspxx(idStr,nameStr);//函数功能：查找商品信息
					JOptionPane.showMessageDialog(CzspxxFrame.this, "查找商品信息成功","提示", JOptionPane.INFORMATION_MESSAGE);
					Dao.czsprecord(LoginDialog.userid,idStr);//增加查找商品信息操作的历史记录，要有商品id
					//}
				}catch(Exception e1) {
					e1.printStackTrace();
				}
		...
		quxiaoButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				int result=JOptionPane.showConfirmDialog(CzspxxFrame.this, "是否确认退出？", "提示", JOptionPane.YES_NO_OPTION);
				if(result==JOptionPane.YES_OPTION) {
					CzspxxFrame.this.dispose();
				}
				else if(result==JOptionPane.NO_OPTION){
					CzspxxFrame.this.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
				}
			}
		...
		this.setTitle("查找商品信息界面");
		this.setIconImage(getToolkit().getImage(getClass().getResource("/resources/seekgoods.PNG")));
