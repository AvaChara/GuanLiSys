...
public class XgspxxFrame1 extends JFrame{
	...
	public XgspxxFrame1(String id,String name,String statue,String type,String date,String deadtime,String produce,String money,String number) {
		...
		deadtimeField.setText(deadtime);
		...
		querenButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					String numberStr=new String(numberField.getText());
					if(isNumeric(moneyStr)!=true||isNumeric(numberStr)!=true) {
						JOptionPane.showMessageDialog(XgspxxFrame1.this,"价格框或数量框输入的不是数字，请输入数字","错误",JOptionPane.ERROR_MESSAGE);
					}else {
						...
						Dao.xgspxx1(idStr,nameStr,statueStr,typeStr,dateStr,deadtimeStr,produceStr,moneyStr,numberStr);//函数功能：增加商品信息
						JOptionPane.showMessageDialog(XgspxxFrame1.this, "修改商品信息成功","提示", JOptionPane.INFORMATION_MESSAGE);
						Dao.xgsprecord(LoginDialog.userid,idStr);//增加修改商品信息操作的历史记录，要有商品id
						//}
						setVisible(false);
					}
		...
		quxiaoButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				int result=JOptionPane.showConfirmDialog(XgspxxFrame1.this, "是否确认退出？", "提示", JOptionPane.YES_NO_OPTION);
				if(result==JOptionPane.YES_OPTION) {
					XgspxxFrame1.this.dispose();
				}
				else if(result==JOptionPane.NO_OPTION){
					XgspxxFrame1.this.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
				}
			}
		...
		this.setTitle("修改商品信息界面");
		this.setIconImage(getToolkit().getImage(getClass().getResource("/resources/amendgoods.PNG")));
