...
public class ScryxxFrame extends JFrame{
	...
	public ScryxxFrame() {
		...
		querenButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					Dao.scryxx(idStr,nameStr);//函数功能：增加商品信息
					JOptionPane.showMessageDialog(ScryxxFrame.this, "删除人员信息成功","提示", JOptionPane.INFORMATION_MESSAGE);
					Dao.scryrecord(LoginDialog.userid,"null");//增加删除人员信息操作的历史记录，不用商品id
					//}
					setVisible(false);
					//}else {
					//JOptionPane.showMessageDialog(ScryxxFrame.this,"输入名字不规范，请输入英文","错误",JOptionPane.ERROR_MESSAGE);
					//}
		...
		quxiaoButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				int result=JOptionPane.showConfirmDialog(ScryxxFrame.this, "是否确认退出？", "提示", JOptionPane.YES_NO_OPTION);
				if(result==JOptionPane.YES_OPTION) {
					ScryxxFrame.this.dispose();
				}
				else if(result==JOptionPane.NO_OPTION){
					ScryxxFrame.this.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
				}
			}
		...
		this.setTitle("删除人员信息界面");
		this.setIconImage(getToolkit().getImage(getClass().getResource("/resources/deleteuser.PNG")));
